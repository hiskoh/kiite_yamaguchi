...
# 名詞抽出＋頻度カウント
# ------------------------
def extract_keywords(meta_data, speaker_filter=None, type_filter=None, tag_filter=None, year_filter=None, min_length=2, stop_words=None):
    tagger = Tagger()
    counter = Counter()
    ...
    for item in meta_data:
        ...
        if tag_filter and tag_filter not in item.get("tags", []):
            continue
        if year_filter and not item.get("date", "").startswith(year_filter):
            continue
...
if meta_data:
    ...
    freq = extract_keywords(meta_data, speaker_filter, type_filter, tag_filter, year_filter, stop_words=stop_words)
