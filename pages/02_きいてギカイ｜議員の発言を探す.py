...
# ========== ▼ 環境チェック（次元・インデックス統計） ======================
with st.expander("🧪 環境チェック：埋め込み次元 & S3 Vectors インデックス統計", expanded=False):
    try:
        # 1) 埋め込み次元（実際に1回だけ生成して長さ表示）
        ...
        st.write({"embedding_model": EMBED_MODEL, "embedding_dim": len(_vec)})
        ...
        desc = s3v_dbg.describe_index(indexArn=S3_INDEX_ARN)
        # ベンダ実装によってキー名は多少違います。無ければst.writeで生出力
        stats = desc.get("statistics") or {}
        st.write({
            "region": AWS_REGION,
            "indexArn": S3_INDEX_ARN,
            "index_stats": stats,   # 例: {"itemCount": 12345, "dimensions": 1536, ...}
            "raw_describe_index": desc
        })
    except Exception as e:
        st.error(f"環境チェック失敗: {e}")
